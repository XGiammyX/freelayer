...
## Per-room aliases — honest limits (TECH-ID-06)

FreeLayer's per-room aliases are a **local, room-scoped, non-cryptographic, metadata-only** convenience — not an identity, membership, role, verification, or anti-impersonation feature:

- A room alias is **not identity, not membership, not a role, not verification, not a `RoomMemberRef`, and not a global username**; a room role is **not** proof of identity, a name collision is **not** proof of impersonation, and disambiguation is **not** verification.
- One class only: a **room presentation alias** (`RoomPresentationAliasV1`) — the local presentation for ONE identity room binding in ONE room (local-only, `not_shared_tech_id_06`, not authenticated, not remotely updatable). Rotation preserves the room identity binding / membership / role / assurance; there is no remote deletion, no authenticated update, and no historical-message relabelling.
- Role, state, and assurance live in a **room member display context** as SEPARATE fields from the alias text; trust and verification are **never** derived from a chosen name.
- Normalization reuses the TECH-ID-05 Unicode pipeline unchanged (NFC + trim; rejecting dangerous control / bidi / zero-width / NUL; ZWJ/ZWNJ retained; not UTS #39) — baseline Unicode validation is **not** full spoofing prevention. There is no automatic reuse from a contact alias, persona label, another room, a root id, or a `RoomMemberRef`.
- Room-local duplicate names force **safe disambiguation** — never `alias_only` on a duplicate, and never exposing a root / global / phone / email / raw-membership id. Cross-room reuse raises a **local redacted warning only** (no room ids or counts); it never denies, merges, or reveals other rooms.
- Policy is strictest-wins, memory/null retention: Bunker/Emergency deny expansive room-alias writes. Network sharing, authenticated binding, public directories, global usernames, persistence, notifications, telemetry, and AI are **structurally unavailable** (6 `identity.room_alias.*` scopes; `check:no-room-alias-bypass`; +12 Policy Matrix rows → 253 specs / 1771 rules; 37 new tests, 711 total). `RoomIdentityBindingV1.roomAliasState` moves off the `"not_implemented_tech_id_06"` placeholder to `RoomAliasBindingStateV1` (`"none" | "local_alias_v1" | "retired"`); the alias record is authoritative, with no direct RoomOS membership mutation.
- Deferred: wire formats/exchange (Gate E), crypto (Gate F), the full Identity Firewall (Gate G), sync (Gate H). Secure Device / ScreenShield / anti-spyware is **externalized** — core keeps only contracts, policy inputs, and honest disclosures.
- Room membership is not identity proof. **DevicePosture is not identity. Not safe for real secrets.**

## Device key model — honest limits (TECH-ID-07)

FreeLayer's device key model is a **local, non-cryptographic, root-subordinate** device-authorization foundation — not an identity, a person, a hardware identifier, DevicePosture, a cryptographic proof, or a remote-management feature:

- A device authorization is **not identity, not a person, not a hardware identifier, not DevicePosture, not a cryptographic proof, and not RoomOS membership**. A device is not a person, a device reference is not a hardware identifier, a device label is not authority, and a local authorization record is not a cryptographic proof.
- One record shape only: a **device authorization record** (`DeviceAuthorizationRecordV1`) referencing exactly ONE identity root, in production `deviceClass:"current_installation_placeholder"`, with `cryptographicallyAuthorized:false`, `hardwareAttested:false`, `remotelySynchronized:false`. Lifecycle `draft_local → active_local_unverified → restricted_local → compromised_suspected → revoked_tombstone` (terminal).
- **Local device references** (`LocalDeviceRef` / `DeviceAuthorizationId` / `DeviceKeySlotId`) are opaque and encode **no** hardware, OS, root, or room data.
- **Scopes** (`identity_root | persona | relationship | room | read_only`) bind exactly one root — no wildcard, no multi-root, no cross-root — and are **attenuation-only** (never widen; read-only never becomes writable). **Capabilities** are local constraints, not proof; attenuation-only, no admin/superuser; `device.add_future` and `recovery.approve_future` are structurally unavailable.
- **Key slots** (`DeviceKeySlotV1`) hold **no key material** — one slot, one purpose (`algorithm:"not_selected_gate_f"`, public/private material false, `keyReuseAcrossPurposes:"forbidden"`); production creates only `reserved_not_generated` slots.
- **Bootstrap** creates exactly ONE current-installation placeholder for a root with no device record; duplicates are rejected and a second device cannot be added. Add / link / import / QR / passport / attestation commands are future-gated and unavailable (TECH-ID-08 + Gate E/F).
- **Revocation** writes a terminal **local** tombstone (`LocalDeviceRevocationResultV1`) — no remote deletion, no session teardown, no message-key or room-key rotation, no data erasure, no cryptographic revocation. **Device revocation is not remote erasure.** Restrict narrows and mark_compromised denies ordinary operations; stale-context checks (`DeviceAuthorizationSnapshotV1` + `assertDeviceAuthorizationSnapshotCurrentV1`) **fail closed** on revision / lifecycle / scope / capability / root / posture drift; root and device compromise are modeled separately and root revocation invalidates child device operations.
- **Labels** are PRIVATE local notes (`visibility:"local_only"`, `peerShared:false`, authority none); a label is not authority and is never networked or logged. Safe summaries keep trust flags false and, in strict modes, redact reference / label / scope / count / timestamps, never exposing hardware / model / OS / DevicePosture / key material.
- **DevicePosture may only tighten** a device decision (never authorize one) and a **RoomOS role never authorizes a device**. A DeviceKey is not DevicePosture; RoomOS membership is not device authorization.
- Policy is strictest-wins, memory/null retention: Bunker/Emergency deny expansive bootstrap; Private/Ghost/Bunker/Emergency deny labels. Real keys, device addition, Device Passport, remote revocation, hardware attestation, persistence, network, notifications, telemetry, and AI are **structurally unavailable** (8 `identity.device.*` scopes; `check:no-device-key-model-bypass`; +14 Policy Matrix rows → 267 specs / 1869 rules; 37 new tests, 748 total).
- The **Device Passport** (future, TECH-ID-08) is a signed root↔device authorization that is **not implemented**. Deferred: crypto / real keys (Gate F), the Device Passport / linking (TECH-ID-08), wire formats/exchange (Gate E), sync (Gate H). Secure Device / ScreenShield / anti-spyware is **externalized** — core keeps only contracts, policy inputs, and honest disclosures.
- RoomOS membership is not device authorization. **DevicePosture is not identity. Not safe for real secrets.**

...
**Per-room aliases** are *local, room-scoped, non-cryptographic, metadata-only* naming foundations under one room identity binding — **not identity, not membership, not a role, not verification, not a `RoomMemberRef`, and not a global username**; a room role is not proof of identity, a name collision is not proof of impersonation, and disambiguation is not verification. `@freelayer/identity` (`packages/identity/src/room-aliases/`) keeps five concepts separate:

- **`RoomPresentationAliasV1`** — the local presentation chosen for ONE identity room binding in ONE room. In TECH-ID-06 it is not shared over any network (`sharingState:"not_shared_tech_id_06"`), not authenticated (`authenticatedBinding:"not_implemented_gate_f"`), and not remotely updatable (Gate E/F). Lifecycle `draft_local → active_local_unshared → retired_tombstone` (terminal); ≤1 active per binding. Rotation retires the old alias and creates a new active-local-unshared alias, preserving the room identity binding / room membership / role / assurance; there is no remote deletion, no authenticated update, and no historical-message relabelling.
- **Room identity binding** — binds a root/persona to a room; it is not a role and not personhood proof. `RoomIdentityBindingV1.roomAliasState` migrates from the `"not_implemented_tech_id_06"` placeholder to `RoomAliasBindingStateV1` (`"none" | "local_alias_v1" | "retired"`); the alias record is authoritative and performs no direct RoomOS membership mutation.
- **Room membership** — RoomOS authorization, a separate concern; an alias never grants or mutates membership.
- **Room member display context** — role / state / assurance carried as SEPARATE fields from the alias text, never derived from it.
- **Trust / verification** — never derived from alias text.

Normalization reuses the TECH-ID-05 Unicode pipeline unchanged (NFC + whitespace trim; rejection of dangerous control / bidi-override / bidi-isolate / selected zero-width / NUL code points; ZWJ (U+200D) / ZWNJ (U+200C) retained; not UTS #39, no confusable detection) — baseline Unicode validation is not full spoofing prevention. There is no automatic reuse from a contact alias, persona label, another room, a root id, or a `RoomMemberRef`. Room-local duplicate-name detection forces safe disambiguation (never `alias_only` on a duplicate; never exposing a root / global / phone / email / raw-membership id), and cross-room reuse is a local redacted warning only (no room ids or counts) that never denies, merges, or exposes other rooms. Policy composition is strictest-wins with memory/null retention — Bunker/Emergency deny expansive room-alias writes. Network sharing, authenticated binding, public directory, global username, persistence, notification, telemetry, and AI are structurally unavailable. This adds 6 `identity.room_alias.*` exact-scope PolicyDecision scopes, +12 Policy Matrix rows (253 specs → 1771 rules), the `check:no-room-alias-bypass` guardrail, and 37 new tests (711 total). **Still not implemented:** wire formats / exchange (Gate E), cryptographic binding (Gate F), the full Identity Firewall (Gate G), and synchronization (Gate H); Secure Device / ScreenShield / anti-spyware remains externalized (core keeps only contracts, policy inputs, and honest disclosures). `RoomMemberRef` is not identity and room membership is not identity proof; **DevicePosture is not identity.**

## Device key model (TECH-ID-07)

The **device key model** is a *local, non-cryptographic, root-subordinate* device-authorization foundation — **not identity, not a person, not a hardware identifier, not DevicePosture, not a cryptographic proof, and not RoomOS membership**; a device is not a person, a device reference is not a hardware identifier, a device label is not authority, and a local authorization record is not a cryptographic proof. `@freelayer/identity` (`packages/identity/src/devices/`) keeps these concepts separate:

- **`LocalDeviceRef` / `DeviceAuthorizationId` / `DeviceKeySlotId`** — opaque local references that encode no hardware, OS, root, or room data; a device reference is not a hardware identifier.
- **`DeviceAuthorizationRecordV1`** — references exactly ONE identity root; in production `deviceClass:"current_installation_placeholder"`. Lifecycle `draft_local → active_local_unverified → restricted_local → compromised_suspected → revoked_tombstone` (terminal), with `cryptographicallyAuthorized:false`, `hardwareAttested:false`, `remotelySynchronized:false`. It is a local constraint, never a cryptographic authorization.
- **Device scope** (`identity_root | persona | relationship | room | read_only`) — binds exactly one root with no wildcard, no multi-root, and no cross-root reach; attenuation-only (never widens; read-only never becomes writable).
- **Device capabilities** — local constraints, not proof; attenuation-only (remove only), no admin/superuser tier; `device.add_future` and `recovery.approve_future` are structurally unavailable.
- **`DeviceKeySlotV1`** — one slot, one purpose; holds no key material (`algorithm:"not_selected_gate_f"`, public/private material false, `keyReuseAcrossPurposes:"forbidden"`); production creates only `reserved_not_generated` slots.
- **Device label** — a PRIVATE local note (`visibility:"local_only"`, `peerShared:false`, authority none); never networked or logged, and never authority.
- **DevicePosture / RoomOS role** — separate concerns: DevicePosture may only tighten a device decision (never authorize one) and a RoomOS role never authorizes a device.

Bootstrap creates exactly ONE current-installation placeholder for a root with no device record (exact PolicyDecision); a duplicate is rejected and a second device cannot be added. Add / link / import / QR / passport / attestation commands are future-gated and unavailable (TECH-ID-08 + Gate E/F). Restrict narrows, mark_compromised denies ordinary operations, and revoke writes a terminal local tombstone (`LocalDeviceRevocationResultV1`) with no remote deletion, no session teardown, no message-key or room-key rotation, no data erasure, and no cryptographic revocation — device revocation is not remote erasure. Stale-context invalidation (`DeviceAuthorizationSnapshotV1` + `assertDeviceAuthorizationSnapshotCurrentV1`) fails closed on revision / lifecycle / scope / capability / root / posture drift; root and device compromise are modeled separately, and root revocation invalidates child device operations. Safe summaries keep trust flags false and, in strict modes, redact reference / label / scope / count / timestamps, never exposing hardware / model / OS / DevicePosture / key material. Policy composition is strictest-wins with memory/null retention — Bunker/Emergency deny expansive bootstrap; Private/Ghost/Bunker/Emergency deny labels. Real keys, device addition, Device Passport, remote revocation, hardware attestation, persistence, network, notification, telemetry, and AI are structurally unavailable. This adds 8 `identity.device.*` exact-scope PolicyDecision scopes, +14 Policy Matrix rows (267 specs → 1869 rules), the `check:no-device-key-model-bypass` guardrail, and 37 new tests (748 total). **Still not implemented:** cryptographic keys / signatures (Gate F), the Device Passport / device linking (TECH-ID-08), wire formats / exchange (Gate E), and synchronization (Gate H); Secure Device / ScreenShield / anti-spyware remains externalized (core keeps only contracts, policy inputs, and honest disclosures). The Device Passport (future, TECH-ID-08) is a signed root↔device authorization and is **not implemented**. RoomOS membership is not device authorization; a DeviceKey is not DevicePosture; **DevicePosture is not identity. Not safe for real secrets.**

...
- **Wiki: enabled as a navigation layer.** The wiki hosts short, friendly overview pages that link into the versioned `docs/` — **canonical truth always stays in `docs/`**, where changes are reviewed like code. Wiki pages must never contradict `docs/`; when in doubt, the wiki links rather than restates. Wiki editing is restricted to collaborators.

...
- **Wiki: live** (<https://github.com/XGiammyX/freelayer/wiki>) as a navigation layer — 10 pages: Home, Getting Started, Architecture Overview, Security and Trust, Roadmap, Comparisons, Contributing, FAQ, plus sidebar/footer. **Canonical truth always stays in `docs/`**; wiki content is authored in the repository's [`wiki/`](../wiki/README.md) directory (reviewed via PR) and synced with `pnpm wiki:publish`, which overwrites direct wiki-UI edits. Wiki pages must never contradict `docs/`.

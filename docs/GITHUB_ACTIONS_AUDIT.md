...
| --- | --- | --- | --- | --- |
| CI | `ci.yml` | push/PR → `main` | `contents: read` | typecheck, lint, test, build + 7 privacy guards (boundaries, external assets, telemetry, forbidden storage, forbidden network, network-deps, build-zero-egress) + doc-link integrity + `audit:privacy`; concurrency-cancelled per ref |

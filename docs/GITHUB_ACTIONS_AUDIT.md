...
| --- | --- | --- | --- | --- |
| CI | `ci.yml` | push/PR → `main` | `contents: read` | typecheck, lint, test, build + 5 privacy guards (boundaries, external assets, telemetry, forbidden storage, forbidden network) + doc-link integrity + `audit:privacy`; concurrency-cancelled per ref |

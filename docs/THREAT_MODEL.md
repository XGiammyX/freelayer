...
**RoomOS policy composition + governance (TECH-22):** policy-precedence attacks (room allow overriding global deny, role over mode, object over room, owner bypassing device requirements), governance escalation (loosening, lowering posture, disabling protections), DevicePosture forgery/staleness (caller-claimed `hardened`/`managed_bunker`, ignored `at_risk`, posture-as-identity), unavailable protected presentation shown anyway, and a malicious client ignoring room policy are the risks. Mitigated by deterministic deny-overrides + strictest-wins, tighten-only monotonic governance, a fail-closed posture resolver (untrusted cannot elevate; `at_risk` tightens; no provider), protected-content denial when integration is absent, posture bound into the authorization revision fence, and content-free reports — with the honest limit that enforcement is local-only and not cryptographically authoritative. Full analysis: [audits/TECH_22_POLICY_COMPOSITION_GOVERNANCE_THREAT_MODEL.md](audits/TECH_22_POLICY_COMPOSITION_GOVERNANCE_THREAT_MODEL.md).

**RoomOS revocation + authorization (TECH-21):** stale authorization (capability resolved before a membership/role/policy/mode change), check/use races (TOCTOU between preparation and execution), cached allow decisions, role-downgrade retention, last-owner races between prepare and execute, cross-scope reuse, and revocation-metadata leakage are the risks; distributed revocation is out of scope (local only). Mitigated by a prepare-vs-execute split with a revision fence, execution-time revalidation as the final gate (no partial effects), capability-set role comparison, restrictive-direction enforcement, current-projection owner-continuity recomputation, a strict no-authorization-cache rule, content-free reports/errors, and explicit local-only non-claims. Full analysis: [audits/TECH_21_REVOCATION_AUTHORIZATION_THREAT_MODEL.md](audits/TECH_21_REVOCATION_AUTHORIZATION_THREAT_MODEL.md).

**RoomOS membership + capability model (TECH-20):** fake identity/authority (forged refs, guessed IDs, role-as-authentication, owner-as-crypto-ownership), privilege escalation (role change, capability widening/forgery, stale/cross-room descriptors), ambient authority + confused deputy, membership-lifecycle abuse (duplicate/resurrection/last-owner removal/stale revision), descriptor-as-bearer-token, membership-graph metadata leakage, and future distributed-revocation confusion are the risks; endpoint capture is out of scope (externalized). Mitigated by deny-by-default ABAC authorization requiring an authentic exact-scope decision, non-authoritative attenuable descriptors bound to room+membership+revision, explicit role→capability tables, the last-owner continuity invariant, lifecycle/revision enforcement, redacted views, and explicit non-claims. Full analysis: [audits/TECH_20_MEMBERSHIP_CAPABILITY_THREAT_MODEL.md](audits/TECH_20_MEMBERSHIP_CAPABILITY_THREAT_MODEL.md).

**RoomOS query model (TECH-19):** query terms, counts, cursors, and result metadata are leakage surfaces; object-level authorization, cross-room/guessed-ID access, view-escalation, search-term leakage, dynamic-regex DoS, cursor-as-authority, and derived-data reappearance (redacted/tombstoned content) are the primary risks; endpoint capture of rendered results is out of scope (externalized). Mitigated by deny-by-default exact-scope authorization, downgrade-only views, immutable frozen snapshots, exact `String.includes` search (no index/regex/history/cache), content-free summaries/errors, non-authority content-free cursors, and side-effect traps. Full analysis: [audits/TECH_19_QUERY_MODEL_THREAT_MODEL.md](audits/TECH_19_QUERY_MODEL_THREAT_MODEL.md).
...
## Identity threats (RESEARCH-ID-01, research)

Identity is not implemented; these are threats the future Identity Firewall must address, analyzed in detail in [audits/RESEARCH_ID_01_IDENTITY_THREAT_MODEL.md](audits/RESEARCH_ID_01_IDENTITY_THREAT_MODEL.md): cross-context **correlation** (global identifiers, stable usernames, shared profiles, alias reuse, invite/recovery/transparency metadata); **impersonation & key substitution** (look-alike aliases, malicious server/relay key swaps, ignored key-change warnings, member-ref/profile mistaken for identity); **multi-device** (unverified/stale/lost devices, root on every device); **recovery** (weak phrases, backup compromise, rollback, administrator backdoor); **alias/invite** (enumeration, homograph, replay, over-scoped or non-expiring invites); and **abuse/Sybil** (disposable identities vs. unlinkability tension). Endpoint compromise (key theft, keyboard/screen/clipboard capture) is **external** (Secure Device) and cannot be solved by the Identity Firewall alone. Assurance is self-asserted; not safe for real secrets.
...
Detailed analysis: [audits/TECH_ID_05_PER_CONTACT_ALIAS_THREAT_MODEL.md](audits/TECH_ID_05_PER_CONTACT_ALIAS_THREAT_MODEL.md). Per-contact aliases and local peer labels are **LOCAL, relationship-scoped, non-cryptographic presentation metadata** — an alias is **not** identity, **not** verification, **not** authentication, **not** a public username, **not** a cryptographic identifier, and does **not** prove a person controls a key. Alias-specific threats and their mitigations: **alias-as-authority / verification** (mitigated: an alias never authorizes — every mutation requires an authentic exact-scope `PolicyDecision`; the display context returns trust state **separately**, so alias text is never the sole identity signal); **cross-relationship correlation via reused alias text** (mitigated: reuse produces a **local WARNING only** — no ids, no counts, user-visible — never an API-provided join; relationship-scoped opaque non-derived ids); **homograph/confusable & bidi spoofing** (mitigated conservatively with Unicode NFC + dangerous-control/bidi/zero-width/NUL rejection, but honestly **NOT a confusable detector** — security-sensitive UI must never rely on alias text alone); **local peer label leakage to the peer** (mitigated: `local_only`, `peerShared:false`, never serialized to a peer, redacted in strict display); **alias used in security-sensitive flows** (mitigated: display context returns trust separately; alias is never the sole identity signal). 7 new `identity.alias.*` exact-scope decision scopes; guardrail `check:no-contact-alias-bypass`. Honest residual: no cryptographic authenticity (Gate F), no wire/remote alias exchange (Gate E), no full Identity Firewall (Gate G), no sync/distributed revocation (Gate H); a local peer label is **NEVER sent to the peer**; DevicePosture is not identity and room membership is not identity proof; Secure Device / ScreenShield / anti-spyware is **EXTERNALIZED**; not safe for real secrets.

## Per-room aliases (TECH-ID-06)

Detailed analysis: [audits/TECH_ID_06_PER_ROOM_ALIAS_THREAT_MODEL.md](audits/TECH_ID_06_PER_ROOM_ALIAS_THREAT_MODEL.md). A room presentation alias (`packages/identity/src/room-aliases/`) is **LOCAL, room-binding-scoped, non-cryptographic presentation metadata** bound to exactly one `RoomIdentityBindingV1` (`not_shared_tech_id_06`, ≤1 active per binding, lifecycle `draft_local → active_local_unshared → retired_tombstone`), reusing the TECH-ID-05 normalization verbatim — an alias is **not** identity, **not** membership, **not** a role, **not** verification, **not** a `RoomMemberRef`, **not** a global/room username or directory handle, and a room role is **not** proof of identity. Room-alias threats and their mitigations: **cross-room correlation** (same alias reused across rooms; a contact alias reused as a room alias; an id encoding root/room/membership; reuse diagnostics exposing the room graph → mitigated: **no automatic reuse** — a room alias comes only from explicit input or an injected placeholder, never a derived copy; **opaque non-encoding ids** carry no root/room/membership id; the cross-room reuse warning is a **local, redacted WARNING** with **no room ids and no counts**, never telemetry — a reused value is not proof of linkage); **in-room spoofing** (duplicate/confusable/bidi aliases; a role embedded in alias text; a collision resolved with a global identifier → mitigated conservatively with **Unicode NFC + bidi/control/zero-width rejection**, but honestly **NOT a confusable detector**; **disambiguation is REQUIRED**; role and assurance are returned as **separate structured fields**, so text saying "owner" confers nothing; disambiguation never uses a global or root id — a collision is **not** proof of impersonation); **authority confusion** (an alias grants membership/role; a rename resets suspension; DevicePosture validates an alias → mitigated: an alias **grants no authority**, every mutation requires an authentic exact-scope `PolicyDecision`, **RoomOS is the membership authority**, rotation preserves role/suspension/assurance, and **DevicePosture may only tighten**, never bless a name); **lifecycle & history** (multiple active per binding; a retired alias reactivated; an alias moved to another room; historical messages relabelled → mitigated: **≤1 active per binding**, **terminal tombstones**, **no cross-binding transfer**, **NO automatic historical-message relabelling**, and no broad plaintext history sink); **room policy & admission** (room policy composes **strictest-wins / deny-overrides** and may **only tighten**, never loosen the Identity Firewall; Bunker assumes **no ScreenShield**, so protected-presentation requirements stay unsatisfied and deny; the sensitive-room admission gate may **restrict** alias-bearing actions but **never grants** identity or membership). 6 new `identity.room_alias.*` exact-scope decision scopes; guardrail `check:no-room-alias-bypass`. Honest residual: no cryptographic alias authenticity (Gate F), no wire/remote alias exchange (Gate E), no device/identity-binding verification (Gate G), no sync / distributed revocation / historical relabelling (Gate H); a room alias is **never shared with the room or its members** and is never an authenticated room profile; DevicePosture is not identity and room membership is not identity proof; an alias **never makes impersonation impossible**; Secure Device / ScreenShield / anti-spyware is **EXTERNALIZED**; not safe for real secrets.

## Device key model (TECH-ID-07)

Detailed analysis: [audits/TECH_ID_07_DEVICE_KEY_THREAT_MODEL.md](audits/TECH_ID_07_DEVICE_KEY_THREAT_MODEL.md). A Device Authorization (`packages/identity/src/devices/`) is a **LOCAL, root-subordinate, non-cryptographic** record asserting that exactly one `LocalIdentityRoot` authorizes one device context — it holds **no key material**, is **`cryptographicallyAuthorized: false`**, and is **not** identity, **not** DevicePosture, **not** a hardware identifier, **not** a person, **not** a `RoomMemberRef`, and **not** a cryptographic proof. Device threats and their mitigations: **root / device confusion** (a subordinate device read as the root authority; a root key copied to every device; a `RoomMemberRef` used as a device credential; the first device self-promoting to permanent authority → mitigated: the device record references **exactly one** root via `DeviceRootRefV1` and is never itself a root; refs are **opaque non-hardware** handles (no serial/model/MAC/IMEI); a device **label carries `authority: "none"`** and is never an authorization input; `RoomMembershipRecordV1` carries **no** device key; the single **bootstrap placeholder is `local_unverified` / `cryptographicallyAuthorized: false`** and **cannot be repeated to add devices** — subordination is asserted locally, not proven (Gate F)); **key-purpose confusion** (one slot reused across identity/messaging/rooms/backups; a signing slot re-read as encryption; a suite/namespace asserted → mitigated: **one slot = one purpose** with `keyReuseAcrossPurposes: "forbidden"`, purpose **immutable after activation**, an unknown/absent purpose **denies** (fail closed), and every slot is an EMPTY typed placeholder (`reserved_not_generated`, `algorithm: "not_selected_gate_f"`) holding **no bytes**); **unauthorized device addition** (a current device enrolls a peer; a UI fabricates an authorized flag; DevicePosture or a RoomOS owner role grants authority; a stale add replays → mitigated: **add / link / passport are future-gated** (`identity.device_key_future` / `identity.device_passport_future`, `effect: "future_gate"`); bootstrap can create only a **new root's** non-authorized placeholder against an **exact-scope `PolicyDecision`**; **DevicePosture may only tighten** and a **RoomOS role is "necessary but never sufficient"** — neither grants device authority; there is **no generic patch / mass-assignment** (`assertExactKeys` + `FORBIDDEN_FIELDS` reject `authenticated` / `trusted` / `verified` / `capabilities`)); **device compromise / loss** (a compromised device stays trusted; old snapshots stay usable; a stale device keeps broad scope, adds devices, or reactivates; root and device compromise conflated → mitigated: **`restrict` narrows** scope monotonically and **bumps `IdentityLocalRevision`**, invalidating stale snapshots; **`mark_compromised` denies ordinary operations**; **`revoke` is a terminal tombstone** that marks the device's key slots revoked and can **never reactivate** (a "new" device is a new record); **root and device compromise are modeled separately**, so marking one does not auto-mark the other); **correlation / metadata** (a hardware serial as the device id; a stable id re-linking independent roots; a device list exposing the persona/room/relationship graph; revocation leaking count/timing → mitigated: **opaque per-context refs** encoding no root/room/membership; **redacted summaries** (strict modes omit ref/label/scope/count/timestamps, trust flags always `false`); **labels are local-only**, never networked or logged; content-free errors and redacted audit). Honest residual: **no real key security, no cryptographic authorization, no remote revocation** (a revoke is a **local** tombstone, not remote erasure), **no cross-device consistency and no synchronized device epoch** (Gate H), **no recovery / backup / export path**, and **no endpoint guarantee** — key custody, exfiltration resistance, and attestation are **external** (Secure Device) or absent. 8 new `identity.device.*` exact-scope decision scopes; +14 Policy Matrix rows; guardrail `check:no-device-key-model-bypass`. A DeviceKey is not DevicePosture; DevicePosture is not identity; a device is not a person; a device reference is not a hardware identifier; a device label is not authority; a local authorization record is not a cryptographic proof; RoomOS membership is not device authorization; device revocation is not remote erasure; real keys / signatures / device-authorization proofs are **Gate F**, the Device Passport / linking is **TECH-ID-08**, formats are **Gate E**, sync / distributed revocation is **Gate H**, Secure Device is **EXTERNALIZED**; not safe for real secrets.

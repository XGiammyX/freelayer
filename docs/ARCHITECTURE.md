...
`packages/identity/src/aliases/` adds two distinct **local, non-cryptographic, metadata-only** per-contact alias classes on the relationship layer. A **pairwise presentation alias** is a relationship-scoped, local-only display name for how the LOCAL user presents within ONE pairwise relationship — NOT shared over any network (`sharingState: "not_shared_tech_id_05"`), NOT authenticated (Gate F), NOT remotely updatable (Gate E/F); ≤1 active per relationship. A **local peer label** is a PRIVATE note the local user attaches to a contact (`visibility: "local_only"`, `peerShared: false`) — **NEVER sent to the peer**; ≤1 active per relationship. Normalization is Unicode NFC + rejection of dangerous control/bidi/zero-width characters (retains ZWJ/ZWNJ; it is **not** case-folding, transliteration, script-blocking, or confusable detection — and not proof of safety). Policy is strictest-wins over memory/null repositories; DevicePosture may only tighten; room policy/role does **not** authorize alias operations. An alias is **NOT** identity, verification, authentication, a public username, or a cryptographic identifier, and does not prove a person controls a key. Machine checks: **+15 Policy Matrix rows (241 specs → 1687 rules)** and the `check:no-contact-alias-bypass` guardrail. Depends only on `@freelayer/privacy` + `@freelayer/security`. Crypto (Gate F), wire formats/exchange (Gate E), the full Identity Firewall (Gate G), and synchronization (Gate H) stay deferred; Secure Device / ScreenShield / anti-spyware is externalized. Room-scoped aliases remain TECH-ID-06. See [audits/TECH_ID_05_PER_CONTACT_ALIAS_THREAT_MODEL.md](audits/TECH_ID_05_PER_CONTACT_ALIAS_THREAT_MODEL.md).

## Per-room aliases (TECH-ID-06)

`packages/identity/src/room-aliases/` adds a **local, non-cryptographic, metadata-only** room presentation alias on the room-binding layer. A **room presentation alias** (`RoomPresentationAliasV1`) names how the LOCAL user presents within ONE identity room binding in ONE Sovereign Room — NOT shared over any network (`sharingState: "not_shared_tech_id_06"`), NOT authenticated (Gate F), NOT remotely updatable (Gate E/F/H); ≤1 active per binding, lifecycle `draft_local` → `active_local_unshared` → `retired_tombstone`. Rotation preserves the binding/membership/role/assurance and performs **no remote deletion, no authenticated remote update, and no historical-message relabelling**. It **reuses the TECH-ID-05 normalization** (Unicode NFC + rejection of dangerous control/bidi/zero-width characters, retaining ZWJ/ZWNJ — not case-folding, transliteration, script-blocking, or confusable detection, and not proof of safety) and never automatically reuses a value from a contact alias, persona label, another room, the root id, or a `RoomMemberRef`. The alias **record** (`RoomAliasStateV1`) is authoritative; the binding's `roomAliasState` (`RoomAliasBindingStateV1`) is a **derived hint**, never authority. Identity **never imports `@freelayer/rooms`**: RoomOS role/state arrive as a **narrow caller-supplied read model** (mirrored `RoomOsMembershipRoleV1`/`RoomOsMembershipStateV1`) referenced only through local `RoomLocalIdRef`/`RoomMembershipIdRef` brands, and RoomOS may consume only a policy-safe room member display context (`RoomMemberDisplayContextV1`) — never the root, persona, or binding internals, and no direct RoomOS membership mutation ever originates in the identity package. A room-local collision requires **safe disambiguation** that never exposes a global/root identifier; cross-room reuse of the same display value yields only a locally redacted warning (no room ids or counts). Policy is strictest-wins over memory/null repositories; DevicePosture and room policy may only **tighten** (Bunker/Emergency deny expansive writes). A room alias is **NOT** identity, membership, a role, verification, authentication, a `RoomMemberRef`, a public/global username, or a cryptographic identifier; a room role is not proof of identity and a collision is not proof of impersonation. Machine checks: **+12 Policy Matrix rows (253 specs → 1771 rules)** and the `check:no-room-alias-bypass` guardrail; 6 `identity.room_alias.*` policy scopes. Depends only on `@freelayer/privacy` + `@freelayer/security`. Crypto (Gate F), wire formats/exchange (Gate E), the full Identity Firewall (Gate G), and synchronization (Gate H) stay deferred; Secure Device / ScreenShield / anti-spyware is externalized and this is not safe for real secrets. See [audits/TECH_ID_06_PER_ROOM_ALIAS_THREAT_MODEL.md](audits/TECH_ID_06_PER_ROOM_ALIAS_THREAT_MODEL.md).

## Device key model (TECH-ID-07)

`packages/identity/src/devices/` adds a **local, non-cryptographic, metadata-only** device-authorization model that is **subordinate to the identity root**. A **device authorization record** (`DeviceAuthorizationRecordV1`) binds exactly **ONE identity root** (`DeviceRootRefV1`, long-lived or ephemeral) to ONE **opaque local device reference** (`LocalDeviceRef` — an internal handle, **not** a hardware identifier, serial, MAC, IMEI, or advertising id). Production may create only the **current-installation bootstrap placeholder** (`deviceClass: "current_installation_placeholder"`, `origin: "local_root_bootstrap_placeholder"`); there is **no second-device linking** — device add / linking / QR approval / import / passport issuance / attestation verification are structurally unavailable and future-gated (**TECH-ID-08** + Gate E/F). A device's reach is bounded by a **scope** that is **attenuation-only** (monotonic: narrow-or-equal, cross-root forbidden, read-only never re-widens) and a set of **capabilities** that are **remove-only** (never re-expanded; `identity.device.add_future`/`identity.recovery.approve_future` are structurally unavailable). Each device may hold **purpose-separated EMPTY key slots** (`DeviceKeySlotV1`) — exactly one purpose per slot, reuse forbidden — that carry **no key material** (`publicMaterialPresent`/`privateMaterialPresent: false`, `algorithm: "not_selected_gate_f"`, `reserved_not_generated`); real key generation is Gate F. Lifecycle is `draft_local` → `active_local_unverified` → `restricted_local` → `compromised_suspected` → `revoked_tombstone` (**terminal**), driven by **restrict / mark-compromised / revoke** transitions, with **stale-context invalidation**: a captured `DeviceAuthorizationSnapshotV1` is re-checked against the authoritative record + root immediately before execution and **fails closed** on any revision/lifecycle/scope/capability drift or root revocation/compromise; stale authorization is never silently refreshed. **Revocation is a terminal LOCAL tombstone** (`LocalDeviceRevocationResultV1`) — it performs **no remote deletion, no remote-session teardown, no message-key or room-key rotation, no data erasure, and no cryptographic revocation** (all `false`). Records are `cryptographicallyAuthorized: false`, `hardwareAttested: false`, `remotelySynchronized: false`; a device **label** is `authority: "none"` local metadata, never posture evidence. State is **memory/null** only (`persistenceClass: "memory_only" | "null"`). Identity **never imports `@freelayer/rooms`**: **RoomOS membership/role never authorizes a device**, and **DevicePosture may only tighten** an already-authorized device (never grant or authorize); RoomOS may **later** consume only a policy-safe **device authorization result**, and Crypto may **later** consume purpose-specific slot **descriptors** after Gate F. Machine checks: **+14 Policy Matrix rows (267 specs → 1869 rules)**, the `check:no-device-key-model-bypass` guardrail, and **8 `identity.device.*` policy scopes** (`bootstrap`, `restrict`, `mark_compromised`, `revoke`, `label.write`, `label.clear`, `summary.read`, `key_slots.read_redacted`). Depends only on `@freelayer/privacy` + `@freelayer/security`. Honest limits: a DeviceKey is **not** DevicePosture; a device is **not** a person; a device reference is **not** a hardware identifier; a device label is **not** authority; a local authorization record is **not** a cryptographic proof; RoomOS membership is **not** device authorization; device revocation is **not** remote erasure — and this is **not safe for real secrets**. Crypto / real keys / signatures / device-authorization proofs (Gate F), the Device Passport (TECH-ID-08), wire formats/exchange (Gate E), and synchronization / distributed revocation (Gate H) stay deferred; Secure Device is externalized. See [audits/TECH_ID_07_DEVICE_KEY_THREAT_MODEL.md](audits/TECH_ID_07_DEVICE_KEY_THREAT_MODEL.md).

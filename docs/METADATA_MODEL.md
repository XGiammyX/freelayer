...
## Per-room alias metadata (TECH-ID-06)

A room presentation alias and its room display context (`packages/identity/src/room-aliases/`, scoped to one `RoomIdentityBindingV1`) are sensitive **LOCAL room metadata**: memory-only (or null), never emitted, never exposed, never networked, and never telemetry. A room alias is **never sent to the room or its members** (`not_shared_tech_id_06`); alias text and ids are redacted in summaries and content-free in errors, and strict modes expose **no exact timestamps**. The cross-room reuse warning exposes **no room graph** — no room ids, no counts (`affectedRoomIdsExposed:false`, `affectedRoomCountExposed:false`), applying the same redaction discipline as the TECH-ID-05 reuse assessment — and collision, reuse, and disambiguation signals are **transient and redacted**, never persisted, networked, or logged (collision exposes `collidingMemberIdsExposed:false`, `exactCollisionCountExposed:false`). A room alias is not identity; a room role is not proof of identity; DevicePosture is not identity; room membership is not identity proof.

## Device key model metadata (TECH-ID-07)

A device authorization record, its key-slot descriptors, device label, and device summary (`packages/identity/src/devices/`, keyed by `DeviceRootRefV1`) are sensitive **LOCAL device metadata**: memory-only (or null), never emitted, never exposed, never networked, and never telemetry. A device reference **never encodes hardware / model / OS / serial / MAC / IMEI or an advertising id** — refs are opaque per-context handles, and `hardwareId` / `deviceFingerprint` / `attestation` are rejected caller fields (no code collects device model, serial, MAC, or advertising ids). A device **label is local-only**, carries `authority: "none"`, and is **never sent to peers** or logged; summaries **omit ids, counts, and timestamps in strict modes** (`redacted:true`; the trust flags `cryptographicallyAuthorized` / `hardwareAttested` / `postureVerified` / `remotelySynchronized` are always `false`), and key-slot views expose **purpose + lifecycle only**, never slot ids or material. Revocation, compromise, and any snapshot signal are **transient and redacted** — never persisted, networked, or logged — and errors are content-free. A DeviceKey is not DevicePosture; DevicePosture is not identity; a device is not a person; a device reference is not a hardware identifier; a device label is not authority; a local authorization record is not a cryptographic proof; RoomOS membership is not device authorization.

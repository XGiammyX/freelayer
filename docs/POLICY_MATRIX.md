...
## TECH-ID-07 rows — device key model

14 rows added (**267 specs → 1869 rules**), split into 9 operational + 5 structural. The **9 operational** device rows — device bootstrap, restrict, mark_compromised, revoke, label set/clear, summary list/detail, and key_slots read_redacted — are `memory_only` (strictest-wins; Bunker/Emergency deny expansive bootstrap/label-set, while restrictive restrict/mark_compromised/revoke/label-clear stay allowed; summary and key_slots reads are redacted). The **5 structural** rows pin the boundaries: `device_add_future`, `device_link_future`, and `device_remote_revoke_future` are **future_gate** (Gates F/E/H, TECH-ID-08); `device_attestation` is **not_implemented**; and `device_history` is **deny**. The existing `identity.device_key_future` (Gate F key generation) and `identity.device_passport_future` (TECH-ID-08) rows are reused — the `device_key_future` rationale is refreshed to note the device MODEL now exists locally while real keys stay Gate F. Device authorizations are local, root-subordinate, non-cryptographic, and metadata-only: a DeviceKey is not DevicePosture and DevicePosture is not identity; a device is not a person; a device reference is not a hardware identifier; a device label is not authority; a local authorization record is not a cryptographic proof; RoomOS membership is not device authorization; and device revocation is not remote erasure (no data erase, no room/message key rotation). All `persistentAllowed:false`, `networkAllowed:false`. 8 new `identity.device.*` PolicySideEffectScope values enforce exact-scope `PolicyDecision`s; IdentityPolicy and the device policy agree (test-enforced); `check:no-device-key-model-bypass` guards the boundary in CI + `audit:privacy`. Secure Device / ScreenShield / anti-spyware remains **externalized**.

...
**All persistent writes must pass through StoragePolicy.** _(Locked — [ADR-0005](adr/ADR-0005-storage-selected-only-by-policy.md).)_
...
- **Capsule spool** — outbound capsules awaiting transport and inbound awaiting processing. Spool honors the strictest mode among the capsules it holds. Spool entries are already-encrypted capsules, but spool _existence and timing_ is metadata → coarse timestamps _(TODO design)_.
...
- **Honest limit:** SSDs (wear leveling, TRIM timing) and journaling filesystems mean overwrite-based secure deletion is unreliable on modern hardware. The design therefore prefers **crypto-shredding**: destroy the keys, and encrypted data becomes unreadable without relying on physical overwrite. _(Depends on encryption-at-rest, Phase 7.)_

## Vault Inspector (future idea)

A built-in screen enumerating everything FreeLayer holds on disk: what, where, under which policy, encrypted how, wipeable how. Doubles as a live, user-verifiable PBOM. _(Phase 7+ design item.)_
...
- **Platform storage betrayal**: PWA storage eviction, OS swap, backup systems (cloud backup silently exfiltrating "encrypted-at-rest" blobs — key management must account for backup exposure). _(TODO research per OS)_
...
- **(TECH-22) Room policy documents, device-posture signals, and admission decisions are memory-only; posture history/persistence is forbidden.** The policy-composition layer introduces these logical data classes, all **memory/null only, never persisted**: `room_policy_document`, `room_policy_projection`, `room_policy_event` (governance events → memory/null log), `room_policy_conflict_report` (transient/redacted), `room_device_posture_signal` (transient), `room_effective_device_posture` (derived transient), `room_sensitive_admission_decision` (transient), `room_protected_presentation_status` (derived transient). `secure_device_evidence_future` is **not implemented**; device-posture **history**, device **inventory**, and remote posture **telemetry** are **forbidden** (matrix `room.device_posture.persist` → deny). Room sensitivity, minimum posture, current posture, at-risk status, policy revision, policy conflicts, and protected-content requirement are sensitive metadata and are not persisted or transmitted in TECH-22.

- **(TECH-21) Prepared authorization contexts and revocation state are transient memory-only; authorization caches are forbidden.** The revocation/authorization layer introduces these logical data classes: `room_prepared_authorization` and `room_authorization_revision` (**transient memory only**, never persisted), `room_revocation_report` (transient/redacted), `room_revocation_event` (memory/null operation log only), and `room_authorization_failure` (no content, not persisted by default). `room_authorization_cache` and `room_distributed_revocation_future` are **forbidden / not implemented** (matrix `room.authorization.cache`/`context_persist`/`context_serialize` → deny; `distributed_revocation`/`signed_revocation` future-gated). No authorization allow-result, capability descriptor, or prepared context may be persisted or reused without current-revision revalidation ([audits/ROOM_AUTHORIZATION_CACHE_AUDIT.md](audits/ROOM_AUTHORIZATION_CACHE_AUDIT.md)).
...
## Device posture / admission data classes (TECH-23)

All Secure Device contract data is **transient (memory-only, current-process)** and never persisted:

| Data class                                        | Policy                      |
| ------------------------------------------------- | --------------------------- |
| `device_posture_assessment`                       | transient memory only       |
| `secure_device_provider_status` / `_capabilities` | transient / static          |
| `sensitive_room_admission_decision`               | transient / redacted        |
| `sensitive_room_session`                          | transient / current-process |
| `protected_content_intent`                        | transient                   |
| `device_attestation_evidence` (future)            | forbidden / not implemented |
| `device_assessment_history`                       | forbidden                   |
...
## Per-room alias storage (TECH-ID-06)

Room presentation alias text (`packages/identity/src/room-aliases/`, one per `RoomIdentityBindingV1`) has a persistence class of **`memory_only` or null** — no persistent plaintext, no disk/browser-storage/database/encrypted storage (encrypted persistence is Gate F), no serialization/export path, and **no persistent alias history, collision index, or reuse index**. Rotation replaces the active presentation and retains **no remote copies**; retire tombstones the record (`retired_tombstone`) and retains **no full alias text** in strict modes. There is **no room bundle export/import** path that would carry aliases, and **historical sender display** — associating a room alias with prior messages — is **deferred to Gate F/H** (no signed, ordered binding history exists to render it). The matrix `identity.room_alias.*` rows deny persistence. A room alias is non-cryptographic presentation metadata — not safe for real secrets.

## Device key model storage (TECH-ID-07)

Device authorization state, key-slot descriptors, and device labels (`packages/identity/src/devices/`, keyed by `DeviceRootRefV1`) have a persistence class of **`memory_only` or null** — no persistent plaintext, no disk/browser-storage/database/encrypted storage (encrypted persistence is Gate F), no serialization/export path, and **no persistent device registry or device history**. There is **no hidden key store**: a key slot holds **NO material** (`reserved_not_generated`, `algorithm: "not_selected_gate_f"`, `publicMaterialPresent:false` / `privateMaterialPresent:false`) — no key bytes, hash, or fingerprint. `restrict` narrows and `revoke` tombstones the record in local memory and retain **no remote copies**; there is **no device import/export** path and **no cloud backup / recovery** that would carry device authorizations or future key material (`storage.write.key_material` is `future_gate`). The matrix `identity.device_key_future` / `identity.device_passport_future` rows hold device addition and the Device Passport future-gated. A device authorization is non-cryptographic, root-subordinate metadata — not safe for real secrets.

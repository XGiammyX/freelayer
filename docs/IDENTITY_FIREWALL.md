...
> **Status: research (RESEARCH-ID-01).** No identity is implemented. There is **no** identity key, recovery, invite, alias, persona, device key, device passport, key transparency, QR verification, or verified-identity claim in FreeLayer today. The identity model is **Gate G** design work; cryptography is **Gate F**. The next step is **TECH-ID-02 — Identity Architecture ADR**. This document records the research status, vocabulary, goals, and unresolved questions — not a shipped system.

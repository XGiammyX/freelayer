...
`@freelayer/identity` now includes a **local, non-cryptographic, root-subordinate** device-authorization model (`packages/identity/src/devices/`). A device authorization is **NOT identity, NOT a person, NOT a hardware identifier, NOT DevicePosture, NOT a cryptographic proof, and NOT RoomOS membership**; a device is not a person, a device reference is not a hardware identifier, a device label is not authority, and a local authorization record is not a cryptographic proof. Several concepts are kept strictly separate:

- **Local device reference** (`LocalDeviceRef` / `DeviceAuthorizationId` / `DeviceKeySlotId`): opaque local ids that encode **no** hardware, OS, root, or room data. A device reference is not a hardware identifier.
- **Device authorization record** (`DeviceAuthorizationRecordV1`): references exactly ONE identity root; in production `deviceClass:"current_installation_placeholder"`. Lifecycle `draft_local → active_local_unverified → restricted_local → compromised_suspected → revoked_tombstone` (terminal), with `cryptographicallyAuthorized:false`, `hardwareAttested:false`, `remotelySynchronized:false` — a local constraint, never a cryptographic authorization.
- **Device scope** (`identity_root | persona | relationship | room | read_only`): binds exactly one root, with **no wildcard, no multi-root, and no cross-root** reach. Scopes are **attenuation-only** — they can only narrow, never widen; a read-only scope is never made writable.
- **Device capabilities**: local constraints, **not proof**; attenuation-only (remove only), with no admin/superuser tier. `device.add_future` and `recovery.approve_future` are **structurally unavailable**.
- **Device key slot** (`DeviceKeySlotV1`): one slot, one purpose; holds **no key material** (`algorithm:"not_selected_gate_f"`, public/private material false, `keyReuseAcrossPurposes:"forbidden"`). Production creates only `reserved_not_generated` slots.
- **Device label**: a PRIVATE local note (`visibility:"local_only"`, `peerShared:false`, authority none). A device label is not authority and is never networked or logged.
- **DevicePosture / RoomOS role**: SEPARATE concerns — DevicePosture may only *tighten* a device decision (never authorize one) and a RoomOS role never authorizes a device. A DeviceKey is not DevicePosture; RoomOS membership is not device authorization.

Bootstrap creates exactly ONE current-installation placeholder for a root that has no device record (exact PolicyDecision); a duplicate is rejected and a second device cannot be added. Add / link / import / QR / passport / attestation commands are **future-gated and unavailable** (TECH-ID-08 + Gate E/F). Restrict narrows a device, mark_compromised denies its ordinary operations, and revoke writes a terminal **local** tombstone (`LocalDeviceRevocationResultV1`) — with **no** remote deletion, no session teardown, no message-key or room-key rotation, no data erasure, and no cryptographic revocation; device revocation is not remote erasure. Stale-context invalidation (`DeviceAuthorizationSnapshotV1` + `assertDeviceAuthorizationSnapshotCurrentV1`) **fails closed** on revision / lifecycle / scope / capability / root / posture drift. Root and device compromise are modeled separately, and root revocation invalidates child device operations. Safe summaries keep trust flags false and, in strict modes, redact reference / label / scope / count / timestamps — and **never** expose hardware / model / OS / DevicePosture / key material. Policy is strictest-wins with memory/null retention: Bunker/Emergency deny expansive bootstrap; Private/Ghost/Bunker/Emergency deny labels. Real keys, device addition, Device Passport, remote revocation, hardware attestation, persistence, network, notification, telemetry, and AI are **structurally unavailable** (8 `identity.device.*` exact-scope PolicyDecision scopes; +14 Policy Matrix rows, 267 specs → 1869 rules; guardrail `check:no-device-key-model-bypass`; 37 new tests, 748 total). The **Device Passport** (future) is a signed root↔device authorization deferred to TECH-ID-08 and **not implemented**. Deferred: crypto / real keys (Gate F), the Device Passport / device linking (TECH-ID-08), wire formats/exchange (Gate E), sync (Gate H). Secure Device / ScreenShield / anti-spyware stays **externalized** — core keeps only contracts, policy inputs, and honest disclosures. RoomOS membership is not device authorization. **DevicePosture is not identity. Not safe for real secrets.**

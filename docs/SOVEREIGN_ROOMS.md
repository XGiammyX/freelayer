...
## Per-contact aliases are relationship-scoped, not room-scoped (TECH-ID-05)

A per-contact **pairwise presentation alias** (`packages/identity/src/aliases/`) is **RELATIONSHIP-scoped** — it names how the local user presents within ONE pairwise relationship — and is deliberately **distinct from any future room-scoped alias (TECH-ID-06)**, which does not exist yet. It is **local-only and NOT shared** over any network; the companion **local peer label** is a private note that is **NEVER sent to the peer**. RoomOS membership and role do **NOT** authorize per-contact alias operations, and neither membership nor role is identity proof — a `RoomMemberRef`/`RoomMembershipIdRef` is a local unverified placeholder, not an identity or a key. An alias is **not** identity, verification, authentication, a public username, or a cryptographic identifier. DevicePosture may only tighten alias operations and never proves identity. Crypto (Gate F), wire formats/exchange (Gate E), the full Identity Firewall (Gate G), and sync (Gate H) stay deferred; Secure Device / ScreenShield / anti-spyware is externalized. See [ARCHITECTURE.md — Per-contact aliases](ARCHITECTURE.md).

## Per-room aliases are bound to one room binding, not one relationship (TECH-ID-06)

A **room presentation alias** (`packages/identity/src/room-aliases/`) is bound to **ONE identity room binding** in **ONE Sovereign Room** — it names how the local user presents in that room — and is deliberately **distinct from a per-contact (relationship-scoped) alias**. It is **local-only and NOT shared** over any network (`sharingState: "not_shared_tech_id_06"`); ≤1 is active per binding, over the lifecycle `draft_local` → `active_local_unshared` → `retired_tombstone`, and rotation preserves the binding/membership/role/assurance with **no remote deletion, authenticated update, or historical-message relabelling**. Room **membership and role do NOT authorize alias operations**, and neither membership nor role is **identity proof** — a `RoomMemberRef`/`RoomMembershipIdRef` is a local unverified placeholder, not an identity or a key. Duplicate aliases within a room **are allowed** but require **safe disambiguation** that never exposes a global or root identifier (never a global/root username, root id, or `RoomMemberRef`); cross-room reuse of the same display value yields only a locally redacted warning (no room ids or counts). A collision is therefore **not proof of impersonation**. Room policy may only **tighten** alias behavior (Bunker/Emergency deny expansive writes; Bunker assumes no ScreenShield) — it never turns an alias into authority: an alias grants **no membership, role, or authority**, and RoomOS may consume only a policy-safe room member display context. An alias is **not** identity, verification, authentication, a public username, or a cryptographic identifier, and a room role is not proof of identity ownership. DevicePosture may only tighten alias operations and never proves identity. Crypto (Gate F), wire formats/exchange (Gate E), the full Identity Firewall (Gate G), and sync (Gate H) stay deferred; Secure Device / ScreenShield / anti-spyware is externalized and this is not safe for real secrets. See [ARCHITECTURE.md — Per-room aliases](ARCHITECTURE.md).

## Device authorization is subordinate to the identity root, not a room (TECH-ID-07)

A **device authorization** (`packages/identity/src/devices/`) is a **local, non-cryptographic** relationship between **ONE identity root** and ONE **opaque local device reference** — it is **subordinate to the identity root, not to a room**. A device is **not a RoomOS member**: a room member (`RoomMemberRef`) is not necessarily one device, and a messaging client may be **device-scoped** (bounded by an attenuation-only scope + remove-only capabilities) **without becoming the user identity**. **RoomOS membership and role do NOT authorize a device** — a `RoomMemberRef`/`RoomMembershipIdRef` is a local unverified placeholder, never a device credential — and a device authorization grants **no membership, role, or authority** in any room. **DevicePosture** (external, tighten-only) **does not authorize a device**: it may only tighten or deny an already-authorized device and never grants, restores, or proves identity. Revocation is a **terminal LOCAL tombstone** — **no remote deletion, no session / message-key / room-key rotation, no data erasure, no cryptographic revocation** — and root revocation/compromise invalidates its subordinate devices by fail-closed re-check, never by remote action. Honest limits: a DeviceKey is **not** DevicePosture; DevicePosture is **not** identity; a device is **not** a person; a device reference is **not** a hardware identifier; a device label is **not** authority; a local authorization record is **not** a cryptographic proof; RoomOS membership is **not** device authorization; device revocation is **not** remote erasure — and this is **not safe for real secrets**. Crypto / real keys (Gate F), the Device Passport (TECH-ID-08), wire exchange (Gate E), and synchronization (Gate H) stay deferred; Secure Device is externalized. See [ARCHITECTURE.md — Device key model](ARCHITECTURE.md).

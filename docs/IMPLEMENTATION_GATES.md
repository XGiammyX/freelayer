...
Until it opens for code: no identity keys, no recovery, no invites, no aliases, no device passports, no key transparency, no QR verification, no directory; `RoomMemberRef` remains a local unverified placeholder; identity state fails closed. **TECH-ID-03 landed the permitted `local, type-safe, crypto-free` scaffolding** (`@freelayer/identity`: roots/personas/relationship + room-binding placeholders, lifecycle validators, policy-gated commands, memory/null repositories) — see [audits/TECH_ID_03_LOCAL_IDENTITY_SCAFFOLDING_AUDIT.md](audits/TECH_ID_03_LOCAL_IDENTITY_SCAFFOLDING_AUDIT.md). Keys/aliases/devices/invites/recovery/sync stay gated on F/E/H. **TECH-ID-04 added the Ephemeral Identity model** (`packages/identity/src/ephemeral/`: independent current-process roots, bounded lifetime, fail-closed expiration, atomic destruction, memory/null repositories) — see [audits/TECH_ID_04_EPHEMERAL_IDENTITY_AUDIT.md](audits/TECH_ID_04_EPHEMERAL_IDENTITY_AUDIT.md). No recovery/promotion/export/synchronization/persistence; no anonymity or forensic-erasure claim. **TECH-ID-05 added Per-Contact Aliases** (`packages/identity/src/aliases/`: **local, relationship-scoped, non-cryptographic, metadata-only** pairwise presentation aliases + private local peer labels) — see [audits/TECH_ID_05_PER_CONTACT_ALIAS_THREAT_MODEL.md](audits/TECH_ID_05_PER_CONTACT_ALIAS_THREAT_MODEL.md). An alias is NOT identity, verification, authentication, a public username, or a cryptographic identifier, and does not prove a person controls a key; a presentation alias is not shared over any network (`not_shared_tech_id_05`), not authenticated, and not remotely updatable; a local peer label is NEVER sent to the peer. Normalization is Unicode NFC + dangerous-control rejection only (retains ZWJ/ZWNJ; no case-fold, transliteration, whole-script blocking, or confusable detection; normalization is not proof an alias is safe). Cryptographic, network-shared, and authenticated aliases stay gated on F/E; per-room aliases are TECH-ID-06; sync stays Gate H. **TECH-ID-06 added Per-Room Aliases** (`packages/identity/src/room-aliases/`: **local, room-binding-scoped, non-cryptographic, metadata-only** room presentation aliases, each bound to ONE `RoomIdentityBindingV1`, ≤1 active per binding) — see [audits/TECH_ID_06_PER_ROOM_ALIAS_THREAT_MODEL.md](audits/TECH_ID_06_PER_ROOM_ALIAS_THREAT_MODEL.md). A room alias is NOT identity, membership, a role, verification, a `RoomMemberRef`, or a global username; a room role is not proof of identity and a collision is not proof of impersonation; duplicate aliases are permitted but require disambiguation (which is not verification), and baseline Unicode validation (reused unchanged from TECH-ID-05) is not full spoofing prevention. No remote alias exchange/authentication, no historical sender-profile system, no alias persistence/history, and no verification by alias; rotation changes no old messages and deletes no peer copies; room aliases are memory/null only and cross-room reuse warnings expose no room graph. The `roomAliasState` placeholder (`not_implemented_tech_id_06`) migrated to `RoomAliasBindingStateV1` (`none`/`local_alias_v1`/`retired`), with the alias record authoritative and the binding state a derived hint. Cryptographic, network-shared, and authenticated room aliases stay gated on F/E; per-room-alias sync stays Gate H; device keys are TECH-ID-07. **TECH-ID-07 added the Device Key Model** (`packages/identity/src/devices/`: **local, root-subordinate, non-cryptographic** device authorization records) — see [audits/TECH_ID_07_DEVICE_KEY_THREAT_MODEL.md](audits/TECH_ID_07_DEVICE_KEY_THREAT_MODEL.md). A DeviceKey is not DevicePosture and DevicePosture is not identity; a device is not a person; a device reference is not a hardware identifier; a device label is not authority; a local authorization record is not a cryptographic proof; RoomOS membership is not device authorization; and device revocation is not remote erasure. No real key material, signatures, or attestation exist; devices are subordinate to identity roots; the current installation is a local unverified placeholder; there is no second-device linking, no Device Passport, and no cross-signing / QR linking / history transfer. Key-purpose slots are descriptors only. Lifecycle is local restrict / mark_compromised / revoke only — revocation is not remote or cryptographic and performs no data erase or room/message key rotation; device state is memory/null only. Real key generation stays Gate F, Device Passport + linking is TECH-ID-08, wire formats Gate E, and multi-device sync Gate H; Secure Device remains external. DevicePosture is not identity; not safe for real secrets.
